...
function BookingCard({ b }: { b: any }) {
  ...
  return (
    <div className={`bg-white rounded-2xl border border-gray-100 px-4 py-3 flex items-center gap-3 ${isActive ? 'border-l-4 border-l-green-500' : ''}`}>
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-green-900 text-sm truncate">{b.loved_ones?.full_name}</p>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap ${STATUS_COLORS[b.status] || 'bg-gray-100 text-gray-500'}`}>
          {STATUS_LABELS[b.status] || b.status}
        </span>
        {isActionable && (
          <Link
            to={`/companion/visit/${b.id}`}
            className="text-xs font-semibold text-green-700 hover:text-green-900 flex items-center gap-0.5 transition-colors"
          >
            {b.status === 'in_progress' ? 'Continue' : 'Start'} <ChevronRight size={12} />
          </Link>
        )}
      </div>
    </div>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-7 gap-1">
              {weekDays.map(day => {
                ...
                return (
                  <div key={day.toISOString()} className={`rounded-xl p-2 text-center ${isToday(day) ? 'bg-green-50' : ''}`}>
                    <p className={`text-[10px] font-semibold ${isToday(day) ? 'text-green-700' : 'text-gray-400'}`}>{format(day, 'EEE')[0]}</p>
                    <p className={`text-xs font-bold mt-0.5 ${isToday(day) ? 'text-green-800' : 'text-gray-500'}`}>{format(day, 'd')}</p>
                    {count > 0 && <span className="block w-1.5 h-1.5 rounded-full bg-green-500 mx-auto mt-1" />}
                  </div>
                )
              })}
            </div>
                  ...
                  return (
                    <div key={day.toISOString()}>
                      <div className="flex items-center gap-2 mb-2">
                        <p className={`text-[11px] font-bold uppercase tracking-widest ${isToday(day) ? 'text-green-700' : 'text-gray-400'}`}>
                          {format(day, 'EEEE, d MMM')}
                        </p>
                        {isToday(day) && (
                          <span className="text-[10px] font-bold bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Today</span>
                        )}
                      </div>
                      {items.length === 0 ? (
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => (
              <p key={d} className="text-[10px] font-semibold text-gray-400 py-1">{d}</p>
            ))}
            {monthDays.map(day => {
              const items   = bookingsOnDay(day)
              ...
              return (
                <button
                  ...
                  className={`aspect-square rounded-xl flex flex-col items-center justify-center gap-0.5 text-sm transition-colors ${
                    ...
                    isToday(day) ? 'bg-green-50 text-green-800 font-semibold' :
                    inMonth   ? 'text-green-900 hover:bg-gray-50' :
          ...
          <div className="space-y-2">
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">{format(selectedDay, 'EEEE, dd MMM yyyy')}</p>
        ...
        bookings.length === 0 ? (
          ...
          <div className="space-y-5">
            {Object.entries(allGroups).map(([date, items]) => (
              <div key={date}>
                <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2">{date}</p>
                <div className="space-y-2">
