-- =====================================================================
-- Seed test data: 1 family (with 1 loved one), 1 companion, 1 booking
-- linking them.
--
-- Lets every /admin, /companion and /dashboard page be checked against
-- real rows instead of empty states.
--
-- - Two new auth.users rows are inserted (seed-family@closeeye.test,
--   seed-companion@closeeye.test). The existing handle_new_user()
--   trigger fires automatically and creates matching public.profiles
--   rows with role='family'.
-- - The seed companion's profile is then promoted to role='companion'
--   (prevent_role_self_escalation is briefly disabled, since this runs
--   as superuser with no auth.uid()/is_admin() session).
-- - A companions row, a loved_ones row, and one booking are inserted
--   directly (SQL Editor bypasses RLS).
--
...
declare
  ...
  v_booking_id uuid;
begin
  -- 1. Seed auth users (handle_new_user() trigger creates matching
  --    public.profiles rows with role='family').
  ...
  -- 2. Promote the seed companion's profile to role='companion' and
  --    set whatsapp numbers on both seed profiles.
  ...
  -- 5. booking linking the loved one to the seed companion.
