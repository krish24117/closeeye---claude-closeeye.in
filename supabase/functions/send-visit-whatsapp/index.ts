...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Send visit_completed template first — opens the 24-h session window so the family
    // can reply in free-form and our team can respond without template restrictions.
    ...
    await Promise.all([...recipients].map((to) =>
      sendWhatsAppTemplate({
        ...
        variables: [familyName, elderName, compProfile?.full_name || 'our companion', visitDate],
        sb,
      })
    ))
