...
function MemberRow({ lo, summary, loading, border }: { lo: LovedOne; summary?: ThreadSummary; loading: boolean; border: boolean }) {
  ...
  return (
    <li className={cn(border && 'border-t border-line')}>
      <Link href={`/family/messages/${lo.id}`} className="flex items-center gap-3.5 px-5 py-4 transition-colors hover:bg-accent-soft/30">
...
export default function MessagesPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Messages"
        subtitle="A private line to your Close Eye team — one conversation for each family member."
      />
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          hint="Add a family member to open a private care conversation with your Close Eye team."
          action={
            <Button asChild>
