...
/**
 * Billing body — the subscription summary + payment history + receipts. Extracted so ONE source
 * renders in both shells: the legacy /family/billing (until it redirects) and the Workspace
...
interface Row { id: string; title: string; date: string | null; amountPaise: number; ref: string | null }
...
function fmt(iso: string | null, region: string = DEFAULT_REGION_CODE): string {
  if (!iso) return '—'
  try { return new Date(iso).toLocaleDateString(localeFor(region), { day: 'numeric', month: 'short', year: 'numeric' }) } catch { return '—' }
}
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section>
        ...
        {error ? (
          <div className="mt-4"><ErrorState title="We couldn’t load your payments" message="Something interrupted the connection — nothing was lost. Please check your connection and try again." onRetry={load} /></div>
        ) : rows === null ? (
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-line/70 bg-card shadow-md">
            {rows.map((r, i) => (
              <div key={r.id} className={cn('flex items-center gap-4 px-5 py-4', i > 0 && 'border-t border-line')}>
                <div className="min-w-0 flex-1"><p className="truncate text-body-sm font-semibold text-ink">{r.title}</p><p className="text-caption text-muted">{fmt(r.date, region)}</p></div>
