...
const STEPS = ['Choose', 'Account', 'Family', 'Activate'] as const

/**
 * The membership purchase progress rail — shown on all four steps of the join
 * funnel (Choose plan → Account → Family → Activate) so the visitor always knows
 * where they are and that they're still inside one continuous task.
 */
export function FunnelSteps({ step }: { step: 1 | 2 | 3 | 4 }) {
  return (
    <ol className="flex items-center justify-center gap-1.5 sm:gap-2.5" aria-label={`Step ${step} of 4`}>
      {STEPS.map((label, i) => {
        const n = i + 1
