  ...
  return (
    <article className={cn('flex flex-col rounded-lg border bg-card p-4 shadow-sm transition-shadow hover:shadow-md', family.liveStatus === 'red' ? 'border-error/30' : 'border-line')}>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        ...
        <Link href={`/console/families/${family.id}`} className="flex min-h-[2.5rem] items-center justify-center gap-1.5 rounded-sm border border-ink/15 text-caption font-semibold text-ink transition-colors hover:border-ink/30 hover:bg-ink/[0.03]"><ArrowRight className="h-3.5 w-3.5 text-green" strokeWidth={1.75} /> Timeline</Link>
