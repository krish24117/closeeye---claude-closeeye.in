...
/** From the admin's seat: our own Close Eye replies sit right (green); the family left. */
function MessageBubble({ m, familyLabel }: { m: Message; familyLabel: string }) {
  const mine = m.sender === 'closeeye'
  return (
    <div className={cn('flex max-w-[82%] flex-col gap-1', mine ? 'items-end self-end' : 'items-start self-start')}>
      {!mine && <span className="px-1 text-caption font-semibold text-green">{familyLabel}</span>}
      <div
        className={cn(
...
/**
 * The Presence Manager's reply view for one family member's thread. Same schema,
 * same realtime, same design tokens as the family thread — perspective flipped
 * so a reply is stored as sender='closeeye' and lands instantly on the family's
 * Messages screen via their existing subscription.
 */
export function AdminMessageThread({ thread }: { thread: AdminThreadRef }) {
  const toast = useToast()
  const familyFirst = thread.familyName.trim().split(/\s+/)[0] || 'Family'
  ...
  const load = useCallback(async () => {
    setStatus('loading')
    try {
      const rows = await fetchThreadAsAdmin(thread.lovedOneId)
      setMessages(rows)
      setStatus('ready')
    } catch {
      setStatus('error')
  ...
  useEffect(() => {
    const unsub = subscribeToThread(thread.lovedOneId, addMessage)
    return unsub
  }, [thread.lovedOneId, addMessage])
  ...
  async function handleFile(file: File | undefined) {
    ...
    setUploading(true)
    try {
      // Upload under the FAMILY's folder so the family can read it back.
      const { path, type } = await uploadAttachment(thread.familyUserId, thread.lovedOneId, file)
  ...
  async function send(e: React.FormEvent) {
    ...
    setSending(true)
    try {
      const created = await sendAdminMessage({
        lovedOneId: thread.lovedOneId,
        familyUserId: thread.familyUserId,
        body,
      ...
      toast('Your reply didn’t send. Please try again.')
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    <div className="flex h-[72vh] min-h-[28rem] flex-col overflow-hidden rounded-lg border border-line bg-card shadow-sm">
      ...
      <div className="flex items-center gap-3 border-b border-line px-5 py-4">
        <Avatar initials={initialsOf(thread.lovedOneName)} alt={thread.lovedOneName} size="md" tone="solid" />
        <div className="min-w-0 flex-1">
          <p className="truncate text-body font-semibold text-ink">{thread.lovedOneName}</p>
          <p className="truncate text-caption text-muted">{thread.familyName}’s family · replying as Close Eye</p>
        </div>
      </div>
        ...
        {status === 'loading' ? (
          ...
          <div className="grid h-full place-items-center px-5">
            <EmptyState
              ...
              hint={`Send the first note to ${familyFirst}’s family about ${thread.lovedOneName.split(' ')[0]}.`}
            />
          </div>
        ) : (
              ...
              return (
                <Fragment key={m.id}>
                  ...
                  {m.sender === 'system' ? (
                    ...
                    <MessageBubble m={m} familyLabel={familyFirst} />
                  )}
                </Fragment>
              )
            })}
      ...
      <form onSubmit={send} className="border-t border-line px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            placeholder={`Reply to ${familyFirst}’s family…`}
            aria-label="Reply"
