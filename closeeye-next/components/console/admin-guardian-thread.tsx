...
import {
  fetchGuardianThreadAsAdmin,
  sendAdminGuardianMessage,
  subscribeToGuardianThread,
  type GuardianMessage,
  type GuardianThreadRef,
...
/** From the staff seat: our Close Eye replies sit right (green); the guardian left. */
function Bubble({ m, guardianLabel }: { m: GuardianMessage; guardianLabel: string }) {
  const mine = m.sender === 'closeeye'
  return (
    <div className={cn('flex max-w-[82%] flex-col gap-1', mine ? 'items-end self-end' : 'items-start self-start')}>
      {!mine && <span className="px-1 text-caption font-semibold text-green">{guardianLabel}</span>}
      <div
        className={cn(
...
/** The Presence Manager's reply view for one guardian's thread. */
export function AdminGuardianThread({ thread }: { thread: GuardianThreadRef }) {
  const toast = useToast()
  const guardianFirst = thread.guardianName.trim().split(/\s+/)[0] || 'Guardian'
  ...
  const load = useCallback(async () => {
    setStatus('loading')
    try {
      const rows = await fetchGuardianThreadAsAdmin(thread.companionId)
      setMessages(rows)
      setStatus('ready')
    } catch {
      setStatus('error')
  ...
  useEffect(() => {
    const unsub = subscribeToGuardianThread(thread.companionId, addMessage)
    return unsub
  }, [thread.companionId, addMessage])
  ...
  async function send(e: React.FormEvent) {
    ...
    setSending(true)
    try {
      const created = await sendAdminGuardianMessage(thread.companionId, body)
      addMessage(created)
      setDraft('')
    } catch {
  ...
  return (
    <div className="flex h-[72vh] min-h-[28rem] flex-col overflow-hidden rounded-lg border border-line bg-card shadow-sm">
      ...
      <div className="flex items-center gap-3 border-b border-line px-5 py-4">
        <Avatar initials={initialsOf(thread.guardianName)} alt={thread.guardianName} size="md" tone="solid" />
        <div className="min-w-0 flex-1">
          <p className="truncate text-body font-semibold text-ink">{thread.guardianName}</p>
          <p className="truncate text-caption text-muted">Guardian{thread.guardianCity ? ` · ${thread.guardianCity}` : ''} · replying as Close Eye</p>
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {status === 'loading' ? (
          ...
          <div className="grid h-full place-items-center px-5"><ErrorState onRetry={() => void load()} message="We couldn’t load this conversation. Please try again." /></div>
        ) : messages.length === 0 ? (
          <div className="grid h-full place-items-center px-5">
            <EmptyState icon={MessageCircle} title="No messages yet" hint={`Send the first note to ${guardianFirst}.`} />
          </div>
        ) : (
              ...
              return (
                <Fragment key={m.id}>
                  ...
                  {m.sender === 'system' ? (
                    ...
                    <Bubble m={m} guardianLabel={guardianFirst} />
                  )}
                </Fragment>
              )
            })}
      ...
      <form onSubmit={send} className="border-t border-line px-4 py-3">
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder={`Reply to ${guardianFirst}…`}
            aria-label="Reply"
