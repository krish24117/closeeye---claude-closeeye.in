# Close Eye — Founder Story (Canonical · single source of truth)

This is the **one** authentic founder story. Use it consistently everywhere — the
founder landing page, marketing website, About page, founder letter, press kit,
investor deck, LinkedIn founder section, and all future collateral. **Do not create
alternate versions.** Shorten only where space demands it (use the short version
below), and never change the underlying narrative.

**In the product, the single source of truth is `lib/content.ts` → `FOUNDER`.** The
founder page, home teaser and founder landing all render from it. This document is
the source for anything outside the codebase.

---

## Why I Built Close Eye

A little while ago, my daughter was born.

It was the happiest day of my life. But it was also one of the quietest.
...
## Short version (space-limited surfaces — e.g. the founder landing)
...
- **Brand name:** the canonical name is **“Close Eye”** (two words), per the Design
  Authority and the product. The original draft used “CloseEye”; normalized here for
  consistency across every surface.
- **Superseded working files:** any earlier founder-story drafts (e.g. the video
  script / build-brief / prototype files under the repo `docs/` folder) are **not**
  the canonical text and should not be used as the story of record.
